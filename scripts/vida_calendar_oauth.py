"""OAuth Google Calendar (lectura + creación de citas) para agente care."""
...
def resolve_creds() -> Path:
    ...
    if not p:
        raise FileNotFoundError("No se encontró gmail_credentials.json en secrets.")
    return p
...
def exchange_code_pkce(*, creds_path: Path, redirect_uri: str, code: str, verifier: str) -> dict:
    ...
    return {
        ...
        "client_id": client["client_id"],
        "client_secret": client["client_secret"],
        "scopes": token_data.get("scope", " ").split() or SCOPES_ALL,
...
def cmd_auth_url() -> dict:
    ...
    flow = Flow.from_client_secrets_file(str(creds_path), scopes=SCOPES_ALL, redirect_uri=REDIRECT)
    ...
    token_path = writable_secret_path("gmail_calendar_token.json")
    ...
    return {
        ...
        "pending": str(PENDING),
        "scopes": SCOPES_ALL,
        "instruction": "Abre auth_url, autoriza, pega la URL callback completa con: exchange --callback-url",
    }
...
def cmd_exchange(callback_url: str) -> dict:
    ...
    return {"status": "ok", "token": str(token_path), "scopes": SCOPES_ALL}


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("cmd", choices=["auth-url", "exchange"])
    ...
    if args.cmd == "auth-url":
        ...
        if not args.callback_url:
            raise SystemExit("--callback-url requerido para exchange")
